...
### 2026-08-19 `787373a` 「モヤモヤしたこと」を任意項目として投稿フォーム・APIに追加
- `postSchema`に`bad`(任意)を追加し、`schema.prisma`の`badThing`(nullable)と整合させる
- フォームに入力欄を追加し、POSTハンドラで`badThing`として保存する

### 2026-08-19 `e5e0c95` design.mdに基づくカラー・フォントトークンを適用
- 用意した`design.md`(ブランドカラー・タイポグラフィ等のデザイン仕様書)を実コードに統合
- `globals.css`の`:root`/`@theme`をdesign.mdのブランドカラー(オレンジ系)・感情カラー(`emotion-*`)に置き換え。このプロジェクトはTailwind v4のCSS-first構成(`tailwind.config.ts`が存在しない)なので、design.mdが前提にしていたv3形式の設定をそのままコピペできず、`globals.css`側への変換が必要だった
- `layout.tsx`で`M PLUS Rounded 1c`(見出し)・`Noto Sans JP`(本文)・`Quicksand`(欧文・数字アクセント)を`next/font/google`で読み込み。日本語フォントは`subsets`に`"japanese"`という選択肢自体が無い(CJKフォントは欧文フォントと配信方式が違う)と分かり、指定せずデフォルトのまま使うことにした
- **つまずいた点・気づき**:
  - shadcn/uiのコンポーネント(Button等)は元から`bg-primary`のような意味ベースのクラス名で書かれており、コンポーネント自体は一切変更していない。`globals.css`側の変数の値を差し替えただけで、使っている箇所全てに一斉に反映される「間接参照」の仕組みだと理解した(Prismaの`@map`と同じ発想)
  - `font-heading`クラスは`CardTitle`にしか付いておらず、投稿ページの`<h2>`には付いていないため、色は変わったがフォントはNoto Sans JPのまま、という「一部だけ反映されている」状態に気づいた
  - ブラウザでの見た目確認に`playwright`を使用(`chromium-cli`が環境に無かったため代替)。確認後はコミット対象から外し、アンインストールして片付けた
...
- 投稿ページの見出し等、`font-heading`が未適用の箇所への反映
